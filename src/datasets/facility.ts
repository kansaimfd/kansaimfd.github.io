 ...
 * 直接開いた利用者は、1施設を見るために88施設ぶん（gzip 171KB）を落としていた。
 ...
 */
function loader<T>(files: Record<string, () => Promise<{ default: T }>>, dir: string) {
  return (id: number): Promise<T> | null => {
    ...
    const pending = load().then(m => m.default)
    cache.set(path, pending)
    return pending
  }
}

export const concertHallFacility = loader(CONCERT_FILES, 'concert')
export const practiceFacility = loader(PRACTICE_FILES, 'practice')

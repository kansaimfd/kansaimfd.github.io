...
describe('出典の 記載なし', () => {
  const 記載なし = (names, overrides = {}) =>
    施設({
      出典: [{ URL: 'https://example.com', 確認日: '2025-06-02', 記載なし: names }],
      ...overrides,
    })

  it('施設と部屋の項目を書ける', () => {
    expect(errorsOf(記載なし(['駐車場', '親子室']))).toEqual([])
    expect(warningsOf(記載なし(['駐車場', '親子室']))).toEqual([])
  })

  it('配列でなければエラー', () => {
    expectOneError(記載なし('駐車場'), /出典\[0\] の 記載なし は配列/)
  })

  // 綴りを誤ると、どの部屋にも配られず黙って消える
  it('書けない項目名はエラー', () => {
    expectOneError(記載なし(['駐車上']), /記載なし に書けない項目があります: "駐車上"/)
    expectOneError(記載なし(['URL']), /書けない項目/)
  })

  it('値のある施設の項目を記載なしにしていたら警告', () => {
    expect(warningsOf(記載なし(['駐車場'], { 駐車場: 30 }))).toEqual([
      {
        kind: '記載なしとした項目に値がある',
        detail: '[concerthall ID:10] テストホール 出典[0] 駐車場',
      },
    ])
  })

  it('掲載される部屋のすべてに値があれば警告、1室でも空いていれば黙る', () => {
    expect(kindsOf(記載なし(['客席数']))).toEqual(['記載なしとした項目に値がある'])
    const 空きあり = 記載なし(['客席数'], {
      部屋: [{ 部屋名: '大ホール', 客席数: 1500 }, { 部屋名: '小ホール' }],
    })
    expect(kindsOf(空きあり)).toEqual([])
    // 会議室は掲載されない部屋なので、そこが空いていても数えない
    const 会議室だけ空き = 記載なし(['客席数'], {
      部屋: [{ 部屋名: '大ホール', 客席数: 1500 }, { 部屋名: '会議室' }],
    })
...
describe('最寄駅', () => {
  ...
  // 公正競争規約の 80m=1分 より短く書かれていたら、実際より近く見せていることになる
  it('駅徒歩が駅距離に対して短すぎれば警告', () => {
    const 施 = 施設({ 最寄駅: [{ 駅: '大阪駅', 駅徒歩: 3, 駅距離: 1000 }] })
    expect(kindsOf(施)).toContain('駅徒歩が駅距離に対して短すぎる')
  })
  ...
  /**
   * 80m/分は健脚の基準で、公式は坂道や信号を織り込んで長めに書く
   * （いずみホールは600mに徒歩10分）。両方向を疑うと常時ノイズになる。
   */
  it('公式が換算より長く書いているぶんには警告しない', () => {
    const 施 = 施設({ 最寄駅: [{ 駅: '大阪駅', 駅徒歩: 10, 駅距離: 600 }] })
    expect(kindsOf(施)).toEqual([])
  })

  it('種別が 駅 / バス停 以外ならエラー', () => {
    expectOneError(施設({ 最寄駅: [{ 駅: '大阪駅', 種別: '電停' }] }), /種別 は 駅 \/ バス停 の/)
  })

  // バス停の名前は「◯◯前」「◯◯バスターミナル」など自由なので、駅の語尾は当てはまらない
  it('バス停には駅名の語尾を求めない', () => {
    expect(kindsOf(施設({ 最寄駅: [{ 駅: 'しづかホール前', 種別: 'バス停' }] }))).toEqual([])
  })

  it('名前がバス停と言っていれば種別が無くても語尾を見ない', () => {
    expect(kindsOf(施設({ 最寄駅: [{ 駅: '小林バス停' }] }))).toEqual([])
  })

  it('駅は従来どおり語尾を見る', () => {
    expect(kindsOf(施設({ 最寄駅: [{ 駅: 'しづかホール前' }] }))).toContain('駅名の語尾が不自然')
  })
...
describe('駅座標マスタとの突き合わせ', () => {
  ...
  // バス停は鉄道の駅ではないのでマスタに載っていない。照合すれば必ず「見つからない」になる
  it('バス停はマスタと照合しない', () => {
    expect(check(施設({ 最寄駅: [{ 駅: 'しづかホール前', 種別: 'バス停' }] }))).toEqual([])
  })

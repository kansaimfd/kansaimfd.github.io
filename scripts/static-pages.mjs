 ...
 * - 全施設の詳細ページがすべて同じ題名・説明になり、SNSに貼っても施設名が出ない
...
/**
 * 説明文に並べる府県。**載っている施設の府県と一致させる。**
 *
 * 以前は「大阪・京都・兵庫・奈良・滋賀・和歌山」と6府県を名乗っていたが、
 * 和歌山県の施設は1件も無い。検索結果から来た人には「和歌山があるはず」に見えるうえ、
 * 一覧には押すと必ず0件になる府県のピルが並んでいた（→ src/options.ts）。
 *
 * 受け入れる府県（validate.mjs の PREF_BOX）はこれまでどおり6府県で、
 * ここはあくまで「いま載っているもの」。**data と食い違えば check-dist.mjs が止める。**
 */
export const DESCRIBED_PREFS = ['大阪', '京都', '兵庫', '奈良', '滋賀']

export const SITE_DESCRIPTION =
  `関西（${DESCRIBED_PREFS.join('・')}）のコンサートホールと音楽練習場を、` +
  '客席数・舞台寸法・設備・最寄駅から探せる非公式のディレクトリです。'

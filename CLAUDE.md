...
- **借りられない施設は `貸館` を立てる**。`休止`＝改修などで一時的、`終了`＝貸館事業そのものを終えた
  （建て替え・用途変更・閉館を含む）。既存の `休館日` は毎週の定休日を指すので語がぶつからないよう
  `貸館` で束ねている。**建物は開いていても貸館だけ終えている施設がある**ため「閉館」とは呼ばない。
...
- **音楽に使えない部屋はビルド時に除外される**。会議室・和室・調理室のように、
  音楽設備（ピアノ・譜面台・管楽器・打楽器・楽器制限・パイプオルガン）も

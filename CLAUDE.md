...
**使えない絞り込み**になる。
**`--coverage` の分母は「部屋」で、一覧に出るホールではない**（concerthall の部屋のうち、
ホールとして一覧に出るのは半分ほど。残りは練習場一覧へ回る練習室など）。そのため客席数や
ホール種別の割合は、部屋で数えると一覧の実感よりかなり低く出る（客席数は部屋で4割台、
一覧のホールでは9割近い）。ホール向けの項目の埋まり具合を
